...
def get_all_users(db: Session, skip: int = 0, limit: int = 100, client_id: Optional[uuid.UUID] = None):
    """
    Lists all users in the system with optional client filter
    ...
    try:
        ...
        logger.info(f"List of users: {len(users)} found")
        return users
...
def admin_update_user(
    ...
    """
    Admin updates user information including client association
    ...
    Args:
        ...
        client_id: New client association (optional)
        is_active: Active status (optional)

    Returns:
        Tuple[Optional[User], str]: Tuple with updated user (or None on error) and status message
    """
    try:
        user = db.query(User).filter(User.id == user_id).first()
        if not user:
        ...
        if email and email != user.email:
            ...
            if existing_user:
                logger.warning(f"Attempt to update user with existing email: {email}")
        ...
        db.commit()
        db.refresh(user)
        logger.info(f"User updated successfully by admin: {user.email}")
...
def admin_reset_password(
    ...
    """
    Admin resets user password without requiring current password
    ...
    try:
        ...
        if not user:
            logger.warning(f"Attempt to reset password for non-existent user: {user_id}")
            return False, "User not found"
...
def update_user_profile(
    db: Session, user_id: uuid.UUID, email: Optional[str] = None, name: Optional[str] = None, language: Optional[str] = None
) -> Tuple[Optional[User], str]:
    """
    ...
    try:
        user = db.query(User).filter(User.id == user_id).first()
        if not user:
            logger.warning(f"Attempt to update profile for non-existent user: {user_id}")
            return None, "User not found"
        ...
        if email and email != user.email:
            existing_user = db.query(User).filter(User.email == email, User.id != user_id).first()
            if existing_user:
                logger.warning(f"Attempt to update to existing email: {email}")
                return None, "Email already in use"
            user.email = email
