...
from src.schemas.user import UserResponse, AdminUserCreate, AdminUserUpdate, AdminResetPassword, MessageResponse
...
async def read_all_users(
    ...
    """
    List all users in the system with optional client filter
    ...
    """
    return get_all_users(db, skip, limit, client_id)
...
async def update_user(
    ...
    Raises:
        HTTPException: If there is an error in update
    """
    # Get current user ID
    current_user_id = payload.get("user_id")
    if not current_user_id:
    ...
    # Update user
    user, message = admin_update_user(
        db,
        user_id,
        name=user_data.name,
...
async def reset_user_password(
    ...
    Raises:
        HTTPException: If there is an error in password reset
    """
    # Get current user ID
    current_user_id = payload.get("user_id")
    if not current_user_id:
    ...
    # Reset password
    success, message = admin_reset_password(db, user_id, password_data.new_password)
    if not success:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=message)
